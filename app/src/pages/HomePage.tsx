...
function HomePage() {
  ...
  const handleRegister = useCallback(async () => {
    ...
    // guard against double-clicks while a previous tx is still in flight
    ...
    try {
      ...
      // pre-flight: maybe the profile already exists (stale UI state, retry after slow network)
      // skip the tx so the user doesn't burn ~0.001 SOL on an init-constraint failure
      const existing = await connection.getAccountInfo(profilePda)
      if (existing) {
        toast({ title: 'Looks like you\'re already registered!', description: 'Refreshing your dashboard…', status: 'info', duration: 2500 })
        setHasProfile(true)
        loadData()
        return
      }
      ...
      } else if (msg.includes('already in use') || msg.includes('custom program error: 0x0')) {
        // race: account got created between our pre-flight check and rpc landing
        toast({ title: 'You\'re already registered!', description: 'Loading your profile…', status: 'info', duration: 2500 })
        setHasProfile(true)
        loadData()
      } else {
  ...
  }, [program, publicKey, connection, registering, toast])
